...
| 4 | Functionality         | Add to Cart Failure - Error User  | User "error_user" cannot add certain products to cart (Sauce Labs Bolt T-Shirt, Sauce Labs Fleece Jacket, Test.allTheThings() T-Shirt). Buttons are unresponsive. | 1. Login as error_user<br>2. Try adding listed products to cart | High     | Blocks purchase of specific items. Fix button handlers. |
| 5 | Functionality / UI    | Sorting Feature Broken - Error User | When sorting products by name or price, a pop-up error appears: "Sorting is broken! This error has been reported to Backtrace." Sorting does not apply. | 1. Login as error_user<br>2. Use sorting dropdown (Name A-Z, Price Low-High, etc.) | High     | Critical usability issue. Fix sorting logic and error handling. |
